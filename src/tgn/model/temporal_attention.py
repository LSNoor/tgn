...
class TemporalAttentionLayer(torch.nn.Module):
  ...
  def forward(self, src_node_features, src_time_features, neighbors_features,
    ...
    # [batch_size, n_node_features] -> [batch_size, 1, n_node_features]
    src_node_features_unrolled = torch.unsqueeze(src_node_features, dim=1)

    # q = [ h_i(t) || phi(t) ]
    # out_dim: [batch_size, 1, query_dim]  (query_dim = n_node_features + time_dim)
    query = torch.cat([src_node_features_unrolled, src_time_features], dim=2)

    #K = [h_j(tj) | | e_ij | | phi(t - tj)]
    # out_dim: [batch_size, n_neighbors, key_dim] (key_dim = n_node_features + n_edge_features + time_dim)
    key = torch.cat([neighbors_features, edge_features, neighbors_time_features], dim=2)

    # No need to permute anymore with 'batch_first=True'
    ...
    # Compute mask of which source nodes have no valid neighbors i.e all the padding is not at zero
    ...
    # We add that for gradient safety
    neighbors_padding_mask_safe = neighbors_padding_mask.clone()

    neighbors_padding_mask_safe[invalid_neighborhood_mask.squeeze(), 0] = False


    # query: [B, 1, Dq], key: [B, N, Dk], mask: [B, N]
    attn_output, attn_output_weights = self.multi_head_target(query=query, key=key, value=key,
                                                              key_padding_mask=neighbors_padding_mask_safe)
    # attn_output : [batch_size, 1, query_dim]
    # attn_output_weights : [batch_size, 1, n_neighbors]
