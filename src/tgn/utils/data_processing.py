...
def get_data(
    ...
    ### Load data and train val test split
    # graph_df: (number of events * [user, items, ts, label, idx])
    graph_df = pd.read_csv(f"./data/ml_{dataset_name}.csv")
    # edge_features: number of edges * dim_edge_features
    edge_features = np.load(f"./data/ml_{dataset_name}.npy")
    # node_features: number of nodes * dim_node_features
    ...
    test_node_set = set(sources[timestamps > val_time]) | set(
        destinations[timestamps > val_time]
    )
    # Sample nodes which we keep as new nodes (to test inductiveness), so we have to remove all
    # their edges from training
    new_test_node_set = set(
        random.sample(sorted(test_node_set), int(0.1 * n_total_unique_nodes))
    )
    ...
    # For training we keep edges happening before the validation time which do not involve any new node
